...
- `654654554686.dkr.ecr.us-east-1.amazonaws.com/devops-ia/production/frontend`

Os manifestos Kubernetes estao organizados em `devops-ia-kubernetes/` com um `kustomization.yaml` na raiz que referencia os recursos de backend e frontend e controla as tags das imagens via bloco `images`.
...
- **Repositorio**: `SalesFX/aws-devops-platform` (monorepo)
...
- Path filtering: mudancas em `devops-ia-terraform/` ou `docs/` nao devem triggerar build de imagens
...
- **Negativas / Trade-offs aceitos**:
  ...
  - O commit automatico do kustomization pode triggerar o workflow novamente se os paths nao forem configurados corretamente -- mitigado com path filter que exclui `devops-ia-kubernetes/`
  - O `GITHUB_TOKEN` precisa de permissao `contents: write` para fazer commit

- **Riscos e mitigacoes**:
  - *Risco*: Loop infinito de triggers (commit do kustomization triggera novo build). *Mitigacao*: Path filter inclui apenas `devops-ia-apps/backend/**` e `devops-ia-apps/frontend/**`, excluindo `devops-ia-kubernetes/`.
...
```mermaid
flowchart TD
    A["Developer push to main<br/>(devops-ia-apps/)"] --> B{"Path filter"}
...
graph LR
    subgraph "GitHub Actions Workflow"
        direction TB
        TRIGGER["on: push<br/>paths: devops-ia-apps/**"]
...
- **Trigger configuration**:
  ...
  - `on.push.paths`: `['devops-ia-apps/backend/**', 'devops-ia-apps/frontend/**']`
...
- **Jobs**:
  ...
  3. **Job `update-kustomization`**:
     ...
     - Steps:
       ...
       c. Usar `kustomize edit set image` ou `sed`/`yq` para atualizar o `newTag` em `devops-ia-kubernetes/kustomization.yaml`
       d. `git add devops-ia-kubernetes/kustomization.yaml`
...
- **Validacoes pos-deploy**:
  - Verificar se a imagem existe no ECR com a tag correta: `aws ecr describe-images --repository-name devops-ia/production/backend --image-ids imageTag=sha-<hash>`
